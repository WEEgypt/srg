...
function Continue() {
    ...
    document.getElementById("achievedstandalone").value = localStorage.getItem("achievedstandalone.store." + x);
    ...
    document.getElementById("achievedabove140").value = localStorage.getItem("achievedabove140.store." + x);
    ...
    document.getElementById("todaystandalone").value = "";
    document.getElementById("todayadsl").value = "";
    document.getElementById("todayabove140").value = "";
...
function NewMonth() {
    ...
    document.getElementById("achievedstandalone").value = "";
    document.getElementById("targetwegold").value = localStorage.getItem("targetwegold.store." + x);
    document.getElementById("achievedadsl").value = "";
    document.getElementById("achievedabove140").value = "";
    document.getElementById("targetadsl").value = localStorage.getItem("targetadsl.store." + x);
...
function GenerateSalesReport() {
    ...
    todaystandaloneInput = document.getElementById("todaystandalone").value.split(" ");
    todaystandalone = 0;
    for (i = 0; i < todaystandaloneInput.length; i++) {
        todaystandalone += parseInt(todaystandaloneInput[i]) || 0;
    }
    todaygoldall = todaygold + todaystandalone || 0;
    if (todaygold == "0") {
        todaygoldreport = "";
    } else {
        todaygoldreport = todaygold + "*FMC ";
    }
    if (todaystandalone == "0") {
        todaystandalonereport = "";
    } else {
        todaystandalonereport = todaystandalone + "*SA ";
    }
    todaygoldreport = todaygoldreport + todaystandalonereport || 0;
    achievedwegold = parseInt(document.getElementById("achievedwegold").value) || 0;
    achievedstandalone = parseInt(document.getElementById("achievedstandalone").value) || 0;
    utdwegold = todaygold + achievedwegold || 0;
    utdstandalone = todaystandalone + achievedstandalone || 0;
    utdwegoldall = utdwegold + utdstandalone || 0;
    ...
    todayabove140Input = document.getElementById("todayabove140").value.split(" ");
    todayabove140 = 0;
    for (i = 0; i < todayabove140Input.length; i++) {
        todayabove140 += parseInt(todayabove140Input[i]) || 0;
    }
    todayadslall = todayadsl + todayabove140 || 0;
    if (todayadsl == "0") {
        todayadslreport = "";
    } else {
        todayadslreport = todayadsl + "*140 ";
    }
    if (todayabove140 == "0") {
        todayabove140report = "";
    } else {
        todayabove140report = todayabove140 + "*HT ";
    }
    todayadslreport = todayadslreport + todayabove140report || 0;
    achievedadsl = parseInt(document.getElementById("achievedadsl").value) || 0;
    achievedabove140 = parseInt(document.getElementById("achievedabove140").value) || 0;
    utdadsl = todayadsl + achievedadsl || 0;
    utdabove140 = todayabove140 + achievedabove140 || 0;
    utdadslall = utdadsl + utdabove140 || 0;
    ...
    dailymobile = todaypayg + todaydata + todaysuperkix + todaytazbeet + todayweclub + todaywemix + todaygold + todaystandalone;
    utdmobile = utdpayg + utddata + utdsuperkix + utdtaz + utdweclub + utdwemix + utdwegoldall;
    ...
    vsGold = ~~Number(Math.round((utdwegoldall / targetwegold) * 100)) || 0;
    vsStandalone = ~~Number(Math.round((utdstandalone / targetwegold) * 100)) || 0;
    vsAdsl = ~~Number(Math.round((utdadslall / targetadsl) * 100)) || 0;
    vsAbove140 = ~~Number(Math.round((utdabove140 / targetadsl) * 100)) || 0;
    ...
    if (todayadsl == "0") {
        ...
        todayadslfordailysales = " + " + todayadslall + " ADSL";
    }
    if (todayfixed == "0") {
    ...
    targetstandaloneAdjusted = targetArrayAdjusted[8];
    targetadslAdjusted = targetArrayAdjusted[9];
    targetabove140Adjusted = targetArrayAdjusted[10];
    ...
    var utdArray = [
        ...
        utdweclub.toString(),
        utdwemix.toString(),
        utdwegoldall.toString(),
        utdstandalone.toString(),
        utdadslall.toString(),
        utdabove140.toString(),
        utdfixed.toString(),
        utdwallet.toString(),
        utdmobile.toString(),
    ];
    ...
    utdwegoldallAdjusted = utdArrayAdjusted[7];
    utdstandaloneAdjusted = utdArrayAdjusted[8];
    utdadslallAdjusted = utdArrayAdjusted[9];
    utdabove140Adjusted = utdArrayAdjusted[10];
    ...
    document.getElementById("salesreport").value =
        ...
        " " +
        utdwegoldallAdjusted +
        " " +
        vsGold +
        "%" +
        "\n" +
        "Gold SA:   " +
        targetstandaloneAdjusted +
        " " +
        utdstandaloneAdjusted +
        " " +
        vsStandalone +
        "%" +
        "\n" +
...
function Save__1() {
    ...
    localStorage.setItem("achievedstandalone.store." + x, utdstandalone);
    ...
    localStorage.setItem("achievedabove140.store." + x, utdabove140);
